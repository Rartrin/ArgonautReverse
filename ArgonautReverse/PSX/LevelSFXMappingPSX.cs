...
{
	public sealed class LevelSFXMappingPSX
	{
		...
		public LevelSFXMappingPSX(IReadOnlyList<string> mapping = null)
		{
			_mapping = mapping;
		}

		public int n_unique_level_sfx => _mapping != null ? _mapping.Count : sounds_hashes.Count;

		public static LevelSFXMappingPSX parse(WadReader data_in, int n_unique_level_sfx)
		{
			var mapping = new string[n_unique_level_sfx];
			for(int i = 0; i < n_unique_level_sfx; i++)
			{
				mapping[i] = data_in.ReadString(16);
			}
		...
		public void serialize(Serializer data_out, LevelSFXContainerPSX level_sfx_groups)
		{
			...
			for(int group_id = 0; group_id < level_sfx_groups.Groups.Count; group_id++)
			{
				...
				var channel_id = channel_group_mapping[group_id];
				...
				for(int sound_id = 0; sound_id < group.Sounds.Count; sound_id++)
				{
					...
					int unique_sound_id = sounds_hashes.Select((i, v) => (i, v)).Where(e => e.v == sound_hash).Select(e => e.i).ElementAt(countValue);
		...
		public void parse_mapping(LevelSFXContainerPSX level_sfx_groups)
		{
			var channel_group_mapping = new Dictionary<int, int>();

			var sounds_hashes = new int[n_unique_level_sfx];
			int group_id = 0;

			for(int channel_id = 1; channel_id < 16; channel_id++)
			{
				bool empty_channel = true;
				for(int unique_sound_id = 0; unique_sound_id < _mapping.Count; unique_sound_id++)
				{
					var sound_id = _mapping[unique_sound_id][channel_id];
					if(sound_id != 255)
					{
						empty_channel = false;
						//TODO: Proper hashing
