...
{
	public sealed class TextureDataPSX
	{
		public record class Box(int _0, int _1, int _2, int _3);

		public readonly TextureFlagsPSX flags;
		public readonly IReadOnlyList<XY> raw_coords;
		public readonly Box cm;
		public readonly int? palette_start;

		public TextureDataPSX(TextureFlagsPSX flags, IReadOnlyList<XY> raw_coords, Box cm, int? palette_start)
		{
			this.flags = flags;
		...
		public static TextureDataPSX parse(WadReader data_in)
		{
			//Bottom Right
			var brX = data_in.Read<byte>();
			var brY = data_in.Read<byte>();

			var palette_info = data_in.Read<ushort>();

			//Bottom Left
			var blX = data_in.Read<byte>();
			var blY = data_in.Read<byte>();

			var flags = (TextureFlagsPSX)data_in.Read<ushort>();

			//Top Right
			var trX = data_in.Read<byte>();
			var trY = data_in.Read<byte>();

			//Top Left
			var tlX = data_in.Read<byte>();
			var tlY = data_in.Read<byte>();
			...
			if((flags & TextureFlagsPSX.IS_NOT_PALETTED) == 0)
			{
			...
			return new TextureDataPSX(flags, raw_coords, cm, palette_start);
		}
		...
		public IReadOnlyList<XY> input_coords
		{
			get
			{
				return round_coords(raw_coords.Select(raw_coord => new XY
				(
						raw_coord.X + 256 / flags.correction_ratio() * flags.n_column(),//Floor divide
						raw_coord.Y + 256 * flags.n_row())
				));
			}
		}
		...
		public IReadOnlyList<XY> output_coords
		{
			...
			{
				var x_correction =
					raw_coords[cm._0].X * flags.correction_ratio()
					- raw_coords[cm._0].X
				;
				return round_coords(raw_coords.Select(raw_coord => new XY
				(
					raw_coord.X + x_correction + 256 * flags.n_column(),
					raw_coord.Y + 256 * flags.n_row()
				)));
			}
		}
		...
		public XY output_top_left_corner => output_coords[cm._0];
	}
}