...
class VAGSoundData(BaseDataClass):
    ...
    def to_vag(self, with_headers: bool = True):
        ...
        header = (
            ...
            + (self.size // self.n_channels).to_bytes(4, "big")#Floor division
        ...
        if self.n_channels == 1:
            ...
            res = bytearray(header_size + self.size // 2)#Floor division
            if with_headers:
                res[:48] = header
            res2 = res.copy()
            for i in range(0, self.size, 2048):
                i2 = header_size + i // 2#Floor division
    ...
    def to_wav(self, filename: str):  # TODO Poor performance
        ...
        audio_data_size = (self.size * 7) // 2#Floor division  # VAG -> WAV has a 3.5 size ratio
        ...
        if self.n_channels == 1:
            ...
            channels = (
                bytearray(audio_data_size // 2),#Floor division
                bytearray(audio_data_size // 2),#Floor division
            )
        ...
        for c in range(self.n_channels):
            ...
            s_2 = 0.0
            samples = [0.0] * 28
            for i in range(16, self.size // self.n_channels, 16):#Floor division
                predict_nr = vag[c][i]
                ...
                for j in range(0, 28, 2):
                    d = vag[c][i + 2 + j // 2]#Floor division
                    s = (d & 0xF) << 12
                    if s & 0x8000:
                ...
                for j in range(28):
                    ...
                    s_1 = samples[j]
                    d = int(samples[j] + 0.5)
                    wav_pos = (i * 7) // 2 + j * 2#Floor division
                    if self.n_channels == 1:
        ...
        if self.n_channels == 2:
            for i in range(0, audio_data_size // 2, 2):#Floor division
